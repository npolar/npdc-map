...
function MapArchiveShowController(
  $scope,
  $controller,
  $routeParams,
  $timeout,
  ...
  function attributionNames(map) {
    let names = [];
    let rightsHolders = (map.contributors || []).filter(c => {
      return c.role === "rightsHolder" && c.name;
    });
  ...
  let show = function() {
    MapArchive.fetch($routeParams, map => {
      ...
      if (map.geometry && map.geometry.bbox.length === 4) {
        ...
        $scope.mapOptions.images = [
          MapImageService.icon($scope.images[0], map)
        ];
      ...
      let relatedQuery = {
        q,
        fields: "id,code,title,type,publication.year,restricted",
        score: true,
        limit: "all"
      };
